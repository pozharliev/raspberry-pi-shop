...
const CheckoutDetailsPayment = ({
	onClick,
	onChange,
	error,
}: {
	onClick: () => void;
	onChange: (data: ICheckoutPayment) => void;
	error: string;
}): JSX.Element => {
	...
	return (
		<>
			...
			<CheckoutDetailsInputContainer>
				<Input big={false} type="text" placeholder="Start Date" value={startDate} onChange={e => setStartDate(e.target.value)} />
			...
			<CheckoutDetailsInputContainer>
				<Input big={false} type="text" placeholder="CVV" autoComplete="cc-csc" value={cvv} onChange={e => setCvv(e.target.value)} />
